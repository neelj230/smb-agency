...
export function buildPhotoManifest(business: BusinessData): PhotoManifest {
  ...
  const priorityOrder = ['team', 'exterior', 'work', 'interior', 'product', 'food']
  let heroImage: PhotoManifest['heroImage']
  for (const cat of priorityOrder) {
    const match = mapped.find(p => p.category === cat)
    if (match) { heroImage = match; break }
  }
  ...
  for (const cat of ['team', 'exterior', 'interior']) {
    ...
    if (match) { aboutImage = match; break }
  }
  if (!aboutImage && aboutCandidates.length > 0) aboutImage = aboutCandidates[0]
  ...
  const galleryImages = mapped.filter(
    p => !usedSrcs.has(p.src) && ['work', 'interior', 'food', 'product'].includes(p.category)
  )
