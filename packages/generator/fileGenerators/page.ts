...
import type { DesignBrief, SectionSelection, GeneratedContent, PhotoManifest } from '../types'
...
function generateImports(sections: SectionSelection[]): string {
  ...
  for (const s of sections) {
    ...
    switch (s.component) {
      case 'ServiceCards': typeImports.add('Service'); break
      case 'TestimonialCarousel': typeImports.add('Review'); break
      case 'TeamGrid': typeImports.add('TeamMember'); break
      case 'FAQAccordion': typeImports.add('FAQItem'); break
      case 'StatsCounter': typeImports.add('Stat'); break
      case 'ProcessSteps': typeImports.add('ProcessStep'); break
    }
  }

  // Always need these types
  typeImports.add('NavLink')
  if (sections.some(s => ['HeroSection', 'AboutSection', 'ImageGallery'].includes(s.component))) {
...
function generateNavLinks(links: { label: string; href: string }[]): string {
  const items = links.map(l => `  { label: '${esc(l.label)}', href: '${l.href}' }`).join(',\n')
  return `const navLinks: NavLink[] = [\n${items},\n]`
}

function generateStats(business: BusinessData): string {
  const items = (business.stats || []).map(s => {
...
function generateTeam(business: BusinessData): string {
  const items = (business.team || []).map(m => {
    const parts = [
      `    name: '${esc(m.name)}'`,
      `    role: '${esc(m.role)}'`,
    ]
    ...
    if (m.credentials?.length) {
      parts.push(`    credentials: [${m.credentials.map(c => `'${esc(c)}'`).join(', ')}]`)
    }
    return `  {\n${parts.join(',\n')},\n  }`
  }).join(',\n')
...
function generateFaq(business: BusinessData): string {
  const items = (business.faq || []).map(f => {
...
function generatePhotoConst(name: string, photo: { src: string; alt: string; category: string }): string {
  return `const ${name}: Photo = {\n  src: '${photo.src}',\n  alt: '${esc(photo.alt)}',\n  category: '${photo.category}',\n}`
}

function generateGalleryPhotos(photos: PhotoManifest): string {
  const items = photos.galleryImages.map(p => {
...
function generateJsx(
  ...
  for (const s of sections) {
    ...
    const jsx = generateSectionJsx(s, business, brief, content, photos, heading)
    if (jsx) {
      if (s.altBackground && s.anchorId !== 'hero') {
        jsxSections.push(`        <div id="${s.anchorId}" className="bg-[var(--brand-bg-alt)]">\n          ${jsx}\n        </div>`)
      } else if (s.anchorId !== 'hero') {
        jsxSections.push(`        <div id="${s.anchorId}">\n          ${jsx}\n        </div>`)
      } else {
...
function generateSectionJsx(
  ...
  switch (section.component) {
    ...
    case 'AboutSection': {
      ...
      const story = business.brandNarrative || business.ownerStory || content.aboutStory || ''
...
function generateHeroJsx(
  ...
  } else if ((variant === 'photo-bg' || variant === 'blurred-reveal') && photos.heroImage) {
