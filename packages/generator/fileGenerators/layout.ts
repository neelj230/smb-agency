...
function fontConstName(family: string): string {
  return family.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '')
}
...
export function generateLayout(
  ...
  const displayImport = FONT_IMPORT_NAMES[displayFont] || displayFont.replace(/\s+/g, '_')
  const bodyImport = FONT_IMPORT_NAMES[bodyFont] || bodyFont.replace(/\s+/g, '_')
  ...
  const displayWeights = brief.font.displayWeights.map(w => `'${w}'`).join(', ')
  ...
  // Keywords
  const keywords = content.keywords.map(k => `'${escapeTsString(k)}'`).join(', ')
...
${displayFont === bodyFont ? '' : `const ${bodyConst} = ${bodyImport}({
  subsets: ['latin'],
  variable: '${bodyVar}',
