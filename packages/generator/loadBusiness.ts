...
export function loadBusiness(slug: string): BusinessData {
  const businessPath = join(process.cwd(), 'data', 'businesses', slug, 'business.json')
  ...
  if (!data.address) throw new Error(`business.json missing "address" for ${slug}`)
...
export function loadDesignDatabase(path?: string) {
  const dbPath = path || join(process.cwd(), 'data', 'design-elements-database.json')
