...
import { selectTemplate, saveUsedTemplate } from "./templateSelector";
import { adaptTemplate } from "./templateAdapter";
...
export async function generate(slug: string): Promise<void> {
  ...
  console.log(
    `  Rating: ${rawBusiness.rating || "N/A"} (${rawBusiness.reviewCount || (rawBusiness.reviews || []).length} reviews)`,
  );
  ...
  console.log(
    `  ${designDb.font_pairings.length} font pairings, ${designDb.color_palettes.length} palettes, ${designDb.heroes.length} heroes`,
  );
  ...
  console.log(
    `  Brand narrative: ${business.brandNarrative ? business.brandNarrative.substring(0, 60) + "..." : "none"}`,
  );
  ...
  console.log(
    `  Font: ${brief.font.display} + ${brief.font.body} (${brief.font.id})`,
  );
  ...
  if (business.services?.length && photos.galleryImages.length > 0) {
    ...
    if (photoIdx > 0)
      console.log(`  Distributed ${photoIdx} photos to services`);
  }

  // 7. Derive nav links (simple — based on available data)
  const navLinks = deriveNavLinks(business);

  // 8. TEMPLATE-BASED GENERATION — Copy best reference site, replace data
  console.log("\n7. Selecting and adapting template...");
  const template = selectTemplate(business, vibe);
  const creative = await adaptTemplate(
    template.slug,
    business,
    vibe,
    photos,
  ...
  saveUsedTemplate(slug, template.slug, business.category);
  ...
  const content = {
    ...
    metaDescription:
      `${business.name} is a ${business.category} located in ${business.city}, ${business.state}. ${business.tagline || ""}`.trim(),
    sectionHeadings: {} as Record<
      string,
  ...
  const files: [string, string][] = [
    ...
    ["app/globals.css", generateGlobalsCss(brief, creative.globalsCssExtras, creative.templateCssVars)],
