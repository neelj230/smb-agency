...
export async function enrichBusiness(business: BusinessData): Promise<BusinessData> {
  ...
  try {
    ...
    const businessPath = join(process.cwd(), 'data', 'businesses', business.slug, 'business.json')
...
function isAlreadyEnriched(business: BusinessData): boolean {
  return !!(
    ...
    business.faq && business.faq.length >= 2 &&
    business.brandNarrative && business.brandNarrative.length > 50
  )
}

async function enrichWithClaude(business: BusinessData): Promise<EnrichmentResult> {
  ...
  const existingServices = (business.services || []).map(s => s.name).join(', ')
  const existingTeam = (business.team || []).map(m => `${m.name} (${m.role})`).join(', ')
  const photoSummary = (business.photos || []).map(p => `${p.category}: ${p.alt}`).join(', ')
...
- Hours: ${business.hours ? Object.entries(business.hours).map(([d, t]) => `${d}: ${t}`).join(', ') : 'Not listed'}
...
function mergeEnrichment(business: BusinessData, enrichment: EnrichmentResult): BusinessData {
  return {
    ...business,
    ...
    services: business.services && business.services.length > 0 ? business.services : enrichment.services,
    team: business.team && business.team.length > 0 ? business.team : enrichment.team,
    faq: business.faq && business.faq.length > 0 ? business.faq : enrichment.faq,
    stats: enrichment.stats.length > 0 ? enrichment.stats : business.stats,
    // Brand colors: only override if empty
    ...((!((business as any).brandColors?.primary)) && enrichment.brandColors.primary
...
function fallbackEnrich(business: BusinessData): BusinessData {
  ...
  const namePattern = /\b([A-Z][a-z]+)\b(?:\s+(?:is|was|has been|helped|did|made|gave|took|always))/g
  const nameMatches: string[] = []
  for (const r of reviews) {
    let match
    while ((match = namePattern.exec(r.text)) !== null) {
      if (!['The', 'They', 'This', 'That', 'Very', 'Great', 'Good', 'Best', 'Will', 'Just', 'Been', 'Have'].includes(match[1])) {
  ...
  const categoryServices: Record<string, { name: string; description: string; icon: string }[]> = {
    pet: [
      { name: 'Full Grooming', description: 'Complete bath, haircut, nail trim, and ear cleaning', icon: 'scissors' },
      { name: 'Bath & Brush', description: 'Refreshing bath with thorough brushing and blow-dry', icon: 'droplets' },
      { name: 'Nail Trimming', description: 'Quick and gentle nail clipping for all breeds', icon: 'hand' },
      { name: 'De-shedding Treatment', description: 'Specialized treatment to reduce shedding and loose fur', icon: 'wind' },
      { name: 'Puppy First Groom', description: 'Gentle introduction to grooming for young pups', icon: 'heart' },
    ],
    plumber: [
      { name: 'Emergency Repairs', description: '24/7 emergency plumbing for leaks, bursts, and backups', icon: 'alert-triangle' },
      { name: 'Drain Cleaning', description: 'Professional drain clearing and prevention', icon: 'pipe' },
      { name: 'Water Heater Service', description: 'Installation, repair, and maintenance of water heaters', icon: 'flame' },
      { name: 'Bathroom Remodeling', description: 'Full bathroom renovation and fixture installation', icon: 'bath' },
      { name: 'Sewer Line Service', description: 'Camera inspection and sewer line repair', icon: 'search' },
    ],
    salon: [
      { name: 'Haircuts & Styling', description: 'Precision cuts and expert styling for every look', icon: 'scissors' },
      { name: 'Color & Highlights', description: 'Vibrant color, balayage, and highlight services', icon: 'palette' },
      { name: 'Blowouts', description: 'Professional blowout and smoothing treatments', icon: 'wind' },
      { name: 'Hair Treatments', description: 'Deep conditioning, keratin, and repair treatments', icon: 'sparkles' },
    ],
    restaurant: [
      { name: 'Dine-In', description: 'Enjoy our full menu in a comfortable atmosphere', icon: 'utensils' },
      { name: 'Takeout', description: 'Fresh food prepared for pickup at your convenience', icon: 'package' },
      { name: 'Catering', description: 'Custom catering for events and special occasions', icon: 'chef-hat' },
      { name: 'Private Events', description: 'Host your next event in our private dining space', icon: 'calendar' },
    ],
    'nail-salon': [
      { name: 'Manicure', description: 'Classic and gel manicure services with premium products', icon: 'hand' },
      { name: 'Pedicure', description: 'Relaxing pedicure treatments for beautiful feet', icon: 'footprints' },
      { name: 'Nail Art', description: 'Custom nail art and creative designs', icon: 'palette' },
      { name: 'Acrylic & Extensions', description: 'Professional nail extensions and acrylic sets', icon: 'sparkles' },
    ],
  }

  // Find matching services
  let services = business.services && business.services.length > 0 ? business.services : []
  if (services.length === 0) {
    const catKey = Object.keys(categoryServices).find(k => cat.includes(k))
    services = catKey ? categoryServices[catKey] : [
      { name: 'Consultation', description: `Free initial consultation to discuss your needs`, icon: 'message-circle' },
      { name: 'Core Services', description: `Professional ${cat} services tailored to you`, icon: 'star' },
      { name: 'Ongoing Support', description: `Continued support and follow-up after service`, icon: 'shield-check' },
    ]
  }
  ...
  const faq: { question: string; answer: string }[] = [
    { question: `What makes ${business.name} different?`, answer: `With a ${business.rating || 5}-star rating from ${business.reviewCount || reviews.length} reviews, our customers consistently praise our quality and service.` },
    { question: 'How do I schedule an appointment?', answer: `Call us at ${business.phone} or use the contact form below. We'll get back to you promptly.` },
    { question: 'What are your hours?', answer: business.hours ? `We're open ${Object.entries(business.hours).slice(0, 2).map(([d, t]) => `${d}: ${t}`).join(', ')} and more. Check our contact section for full hours.` : `Contact us at ${business.phone} for our current hours.` },
  ]

  // Build stats
  const stats = [
    { value: business.reviewCount || reviews.length, suffix: '+', label: 'Happy Customers' },
    { value: business.rating || 5, suffix: '/5', label: 'Average Rating' },
  ]
  if (reviews.length > 0) {
    const earliest = reviews.reduce((min, r) => r.date && r.date < min ? r.date : min, reviews[0]?.date || '')
    if (earliest) {
      const years = new Date().getFullYear() - new Date(earliest).getFullYear()
      if (years > 1) stats.push({ value: years, suffix: '+', label: 'Years Serving ' + city })
    }
  }
