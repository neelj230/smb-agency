...
import type {
  VibeProfile, DesignBrief, DesignDatabase,
  FontPairingSelection, PaletteSelection, HeroSelection,
  AnimationSelection, NavSelection, DbFontPairing, DbColorPalette,
} from './types'
import { selectByVibe, colorDistance, slugHash, pickByHash, vibeMatchScore } from './utils'
...
export function selectDesign(
  ...
  // --- Font pairing (weighted random from top candidates) ---
  const googleFonts = db.font_pairings.filter(
    f => !f.next_font_import.includes('local') && !f.next_font_import.includes('Local')
  )
  const selectedFont = weightedRandomSelect(googleFonts, profileVibes, slug, 0)
  ...
  // --- Color palette (weighted random, with brand color preference) ---
  const palette = selectPalette(db.color_palettes, profileVibes, business, slug)

  // --- Hero variant ---
  const hero = selectHero(db.heroes, profileVibes, business, slug)
  ...
  return { font, palette, hero, nav, animations, cssRecipes, componentVariants }
}
...
function selectPalette(
  ...
  if (brandColors?.primary && brandColors.primary.startsWith('#')) {
    ...
    selected = topCandidates[(slugHash(slug) + 1) % topCandidates.length].palette
  } else {
...
function normalizePaletteColors(colors: Record<string, string>): Omit<PaletteSelection, 'id'> {
  const primary =
    colors.primary ||
    colors.primary_red ||
...
function selectHero(
  ...
  candidates.push({
    variant: 'centered',
    vibeScore: profileVibes.includes('clean') || profileVibes.includes('minimal') ? 3 : 1,
  })
  candidates.push({
    variant: 'dark-bold',
    vibeScore: profileVibes.includes('bold') || profileVibes.includes('premium') ? 3 :
               profileVibes.includes('modern') ? 2 : 0,
  })
  ...
  if (hasTeamPhoto || hasExteriorPhoto) {
    candidates.push({
      variant: 'split',
      vibeScore: profileVibes.includes('professional') || profileVibes.includes('friendly') ? 3 :
                 profileVibes.includes('trustworthy') || profileVibes.includes('personal') ? 2 : 1,
    })
  }
  ...
  if (hasAnyPhoto) {
    candidates.push({
      variant: 'photo-bg',
      vibeScore: profileVibes.includes('visual') || profileVibes.includes('immersive') ? 3 :
                 profileVibes.includes('warm') || profileVibes.includes('inviting') ? 2 : 1,
    })
    candidates.push({
      variant: 'blurred-reveal',
      vibeScore: profileVibes.includes('premium') || profileVibes.includes('elegant') ? 3 :
                 profileVibes.includes('creative') ? 2 : 0,
    })
  }
  ...
  candidates.sort((a, b) => {
    ...
    return (slugHash(slug + a.variant) % 100) - (slugHash(slug + b.variant) % 100)
  })

  // Pick from top 3 candidates using weighted selection
  const topCandidates = candidates.slice(0, Math.min(3, candidates.length))
  const chosen = topCandidates[(slugHash(slug + 'hero') + 7) % topCandidates.length]
...
function selectNav(profileVibes: string[], hero: HeroSelection): NavSelection {
  if (hero.variant === 'dark-bold' || hero.variant === 'photo-bg' || hero.variant === 'blurred-reveal') {
    return { variant: 'dark' }
  }
...
function selectAnimations(
  ...
  const result: AnimationSelection[] = [
    { id: 'fade-up-on-scroll', name: 'Fade Up on Scroll', category: 'entrance', complexity: 'simple' },
    { id: 'staggered-children', name: 'Staggered Children', category: 'entrance', complexity: 'simple' },
  ]
  ...
  if (extras.length > 0) {
    ...
    const isMinimal = profileVibes.includes('minimal') || profileVibes.includes('clean')
    ...
    if (pool.length > 0) {
      ...
      for (let i = 0; i < count; i++) {
        const pick = pickByHash(pool.filter(a => !used.has(a.id)), slug, 10 + i)
        if (pick) {
          used.add(pick.id)
          result.push({
            id: pick.id,
...
function selectComponentVariants(profileVibes: string[], slug: string): Record<string, string> {
  const variants: Record<string, string> = {}

  // ServiceCards — weighted selection
  if (profileVibes.includes('editorial') || profileVibes.includes('premium') || profileVibes.includes('elegant')) {
    variants['ServiceCards'] = 'alternating'
  } else {
    variants['ServiceCards'] = pickByHash(['grid', 'grid', 'alternating'], slug, 20) // 2/3 chance grid
  }

  // TestimonialCarousel — more variety
  if (profileVibes.includes('established') || profileVibes.includes('trustworthy')) {
    variants['TestimonialCarousel'] = 'featured'
  } else if (profileVibes.includes('energetic') || profileVibes.includes('warm') || profileVibes.includes('playful')) {
    variants['TestimonialCarousel'] = 'scroll'
  } else {
    variants['TestimonialCarousel'] = pickByHash(['scroll', 'grid', 'featured'], slug, 3)
  }

  // ProcessSteps
  if (profileVibes.includes('premium') || profileVibes.includes('elegant') || profileVibes.includes('personal')) {
  ...
  // StatsCounter
  if (profileVibes.includes('bold') || profileVibes.includes('premium') || profileVibes.includes('dark')) {
  ...
  } else if (profileVibes.includes('energetic') || profileVibes.includes('playful')) {
    variants['ImageGallery'] = 'scroll'
  } else {
    variants['ImageGallery'] = pickByHash(['grid', 'scroll', 'masonry'], slug, 6)
  }
