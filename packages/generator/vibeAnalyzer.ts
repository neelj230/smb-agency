...
const SENTIMENT_KEYWORDS: Record<string, string[]> = {
  trustworthy:  ['honest', 'reliable', 'trust', 'trusted', 'integrity', 'dependable', 'straightforward', 'transparent'],
  premium:      ['beautiful', 'amazing', 'stunning', 'incredible', 'exceptional', 'outstanding', 'excellent', 'luxury', 'gorgeous'],
  friendly:     ['friendly', 'nice', 'welcoming', 'kind', 'pleasant', 'cheerful', 'warm', 'sweet', 'lovely'],
  professional: ['professional', 'knowledgeable', 'expert', 'thorough', 'meticulous', 'skilled', 'precise'],
  direct:       ['fast', 'quick', 'responsive', 'prompt', 'efficient', 'timely', 'immediate', 'same-day'],
  personal:     ['family', 'care', 'caring', 'personal', 'attentive', 'dedicated', 'love', 'gentle', 'patience'],
  established:  ['years', 'since', 'generation', 'decades', 'experience', 'veteran', 'longtime', 'always'],
  playful:      ['fun', 'happy', 'laugh', 'enjoy', 'love', 'best', 'awesome', 'fantastic'],
  calm:         ['relaxing', 'calm', 'peaceful', 'soothing', 'comfortable', 'zen', 'tranquil'],
  creative:     ['creative', 'unique', 'custom', 'artistic', 'original', 'innovative', 'design'],
}

/** Negation words that invert sentiment */
const NEGATION_WORDS = ['not', 'no', 'never', "don't", "doesn't", "didn't", "wasn't", "weren't", "isn't"]
...
export function analyzeVibe(business: BusinessData): VibeProfile {
    ...
    for (const kw of keywords) {
      ...
      if (matches) {
        ...
        for (const reviewText of reviewTexts) {
          ...
          if (kwIdx > 0) {
            const prefix = reviewText.substring(Math.max(0, kwIdx - 15), kwIdx).trim()
            const isNegated = NEGATION_WORDS.some(neg => prefix.endsWith(neg))
            if (!isNegated) hits++
          } else if (kwIdx === 0) {
            hits++
          }
  ...
  for (const kw of personalityKeywords) {
    ...
    if (normalized in scores || Object.keys(SENTIMENT_KEYWORDS).includes(normalized)) {
      addScore(normalized, 2)
    }
    // Map common personality descriptors to vibes
    if (['gentle', 'patient', 'caring'].includes(normalized)) addScore('calm', 2)
    if (['fun', 'playful', 'energetic'].includes(normalized)) addScore('playful', 2)
    if (['skilled', 'expert', 'precise'].includes(normalized)) addScore('professional', 2)
  ...
  const workPhotos = photos.filter(p => p.category === 'work' || p.category === 'product')
  ...
  if (websiteContent) {
    ...
    if (wc.includes('luxury') || wc.includes('premium') || wc.includes('exclusive')) addScore('premium', 2)
    if (wc.includes('family') || wc.includes('community') || wc.includes('neighborhood')) addScore('warm', 2)
    if (wc.includes('modern') || wc.includes('innovative') || wc.includes('cutting-edge')) addScore('modern', 2)
    if (wc.includes('tradition') || wc.includes('heritage') || wc.includes('since')) addScore('established', 2)
  }

  // Sort vibes by score, tiebreak with slug hash
  const slug = business.slug || ''
  const sorted = Object.entries(scores)
    .sort(([vibeA, scoreA], [vibeB, scoreB]) => {
