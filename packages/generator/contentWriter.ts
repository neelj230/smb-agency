...
export async function generateContent(
  ...
  try {
    ...
    console.warn(
      `Claude API call failed, using template fallback: ${(err as Error).message}`,
    );
...
Return ONLY valid JSON:
{
  ...
  "sectionHeadings": {
    ${sectionIds
      ...
      .map(
        (id) =>
          `"${id}": { "heading": "creative heading", "subheading": "optional punchy one-liner" }`,
      )
      .join(",\n    ")}
  }${
    needsAboutStory
      ? `,
  "aboutStory": "2-3 paragraphs. Weave in real details from reviews. Name staff. Tell a story, not a corporate bio."`
      : ""
  }${
    needsProcessSteps
      ? `,
...
function generateFallback(
  ...
  if (business.brandNarrative) {
    ...
    const topReview = (business.reviews || []).sort(
    ...
    switch (s.anchorId) {
      case "services":
        sectionHeadings.services = {
          heading: `What ${business.name} Does Best`,
        };
        break;
      case "about":
        sectionHeadings.about = { heading: "The Story" };
        break;
      case "process":
        sectionHeadings.process = { heading: "How We Work Together" };
        break;
      case "reviews":
        sectionHeadings.reviews = { heading: "Straight From Our Customers" };
        break;
      case "gallery":
        sectionHeadings.gallery = { heading: "See For Yourself" };
        break;
      case "team":
        sectionHeadings.team = { heading: "The People Behind It All" };
        break;
      case "faq":
        sectionHeadings.faq = { heading: "Questions? We've Got Answers" };
        break;
      case "contact":
        sectionHeadings.contact = { heading: "Let's Talk" };
        break;
      case "founder":
        sectionHeadings.founder = { heading: "A Word From Us" };
        break;
    }
  }
  ...
  const processSteps = sections.some((s) => s.anchorId === "process")
    ? [
        {
          title: "Reach Out",
          description: `Call ${business.phone} or fill out our contact form — we respond fast.`,
        },
        {
          title: "We Listen",
          description:
            "Tell us what you need. We tailor our approach to your situation.",
        },
        {
          title: "We Deliver",
          description: "Our team gets to work with care and precision.",
        },
        {
          title: "You Love It",
          description:
            "We follow up to make sure everything exceeds your expectations.",
        },
      ]
    : undefined;
  ...
  return {
    ...
    sectionHeadings,
    aboutStory:
      !business.brandNarrative && !business.ownerStory
