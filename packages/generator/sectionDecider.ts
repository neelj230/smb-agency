...
const SERVICE_CATEGORIES = [
  ...
  'cleaning', 'landscaping', 'auto-repair', 'painter',
]
...
const SECTION_ORDERS: Record<string, string[]> = {
  ...
  'trust-first': ['hero', 'stats', 'reviews', 'services', 'about', 'process', 'gallery', 'team', 'founder', 'faq', 'contact'],
  // Services-first: lead with what you do
  'services-first': ['hero', 'services', 'stats', 'about', 'process', 'reviews', 'gallery', 'team', 'founder', 'faq', 'contact'],
  // Story-first: lead with narrative
  'story-first': ['hero', 'about', 'stats', 'services', 'gallery', 'reviews', 'process', 'team', 'founder', 'faq', 'contact'],
  // Visual-first: lead with gallery
  'visual-first': ['hero', 'gallery', 'stats', 'services', 'about', 'reviews', 'process', 'team', 'founder', 'faq', 'contact'],
}

function pickSectionOrder(vibes: string[], slug: string): string[] {
  if (vibes.includes('trustworthy') || vibes.includes('established')) return SECTION_ORDERS['trust-first']
  if (vibes.includes('visual') || vibes.includes('creative')) return SECTION_ORDERS['visual-first']
  if (vibes.includes('personal') || vibes.includes('warm')) return SECTION_ORDERS['story-first']
...
export function decideSections(
  ...
  const available = new Map<string, { component: string; variant?: string; props: Record<string, unknown> }>()

  // Hero — always
  available.set('hero', { component: 'HeroSection', variant: brief.hero.variant, props: {} })

  // Stats — if available
  if (business.stats && business.stats.length >= 2) {
    available.set('stats', { component: 'StatsCounter', variant: brief.componentVariants['StatsCounter'], props: {} })
  }

  // Services or Menu
  if (MENU_CATEGORIES.includes(cat)) {
    available.set('services', { component: 'ServiceCards', variant: brief.componentVariants['ServiceCards'], props: {} })
  } else if (business.services && business.services.length >= 1) {
    const columns = business.services.length <= 4 ? 2 : 3
    available.set('services', { component: 'ServiceCards', variant: brief.componentVariants['ServiceCards'], props: { columns } })
  }
  ...
  if (SERVICE_CATEGORIES.includes(cat) || cat.includes('pet') || cat.includes('groom') || cat.includes('salon')) {
    available.set('process', { component: 'ProcessSteps', variant: brief.componentVariants['ProcessSteps'], props: {} })
  }

  // Testimonials — if 3+ reviews
  if (business.reviews && business.reviews.length >= 3) {
    available.set('reviews', { component: 'TestimonialCarousel', variant: brief.componentVariants['TestimonialCarousel'], props: {} })
  }
  ...
  const galleryPhotos = (business.photos || []).filter(
    p => p.category === 'work' || p.category === 'interior' || p.category === 'food'
  )
  if (galleryPhotos.length >= 3) {
    available.set('gallery', { component: 'ImageGallery', variant: brief.componentVariants['ImageGallery'], props: {} })
  }
...
export function deriveNavLinks(sectionsOrBusiness: SectionSelection[] | BusinessData): { label: string; href: string }[] {
  ...
  if (!Array.isArray(sectionsOrBusiness)) {
    ...
    if ((business.services || []).length > 0) links.push({ label: 'Services', href: '#services' })
    links.push({ label: 'About', href: '#about' })
    if ((business.reviews || []).filter(r => r.text && r.text.trim().length >= 20).length >= 3) {
      links.push({ label: 'Reviews', href: '#reviews' })
    }
    if ((business.photos || []).length >= 5) links.push({ label: 'Gallery', href: '#gallery' })
    if ((business.faq || []).length > 0) links.push({ label: 'FAQ', href: '#faq' })
  ...
  const navSections = ['services', 'about', 'reviews', 'gallery', 'team', 'faq', 'contact']
