...
export async function runQualityChecks(
  ...
  if (passed) {
    console.log(
      `  Design: fonts(${brief.font.display}+${brief.font.body}), palette(${brief.palette.id}), hero(${brief.hero.variant})`,
    );
  }
...
function auditContent(siteDir: string, business: BusinessData): CheckResult[] {
  ...
  results.push({
    ...
    message: layout.includes(business.name)
      ? "Found in layout"
      : page.includes(business.name)
        ? "Found in page"
  ...
  results.push({
    ...
    message: page.includes(business.phone)
      ? "Found"
      : `"${business.phone}" not found`,
    severity: "warning",
  });

  // No placeholder text (but allow HTML placeholder="" attributes on form inputs and JSX comments)
  const pageWithoutAttrs = page
  ...
  const foundPlaceholder = placeholders.find((p) =>
    pageWithoutAttrs.toLowerCase().includes(p.toLowerCase()),
  );
  results.push({
  ...
  if (business.reviews && business.reviews.length > 0) {
    ...
    results.push({
      ...
      message: reviewMatches
        ? "Reviews match source data"
        : "Review text may be modified",
      severity: "warning",
    });
  }
...
function auditContentRichness(business: BusinessData): CheckResult[] {
  ...
  results.push({
    ...
    message:
      serviceCount >= 3
        ...
        : `Only ${serviceCount} services (need 3+)`,
    severity: "warning",
  });

  // About story check
  const aboutLen =
    (business.brandNarrative || "").length + (business.ownerStory || "").length;
  results.push({
  ...
  results.push({
    ...
    passed: faqCount >= 2,
    message:
      faqCount >= 2 ? `${faqCount} items` : `Only ${faqCount} items (need 2+)`,
    severity: "warning",
  });
  ...
  if (
    (business.photos || []).filter((p) =>
      ["work", "interior", "food"].includes(p.category),
    ).length >= 3
  )
    sectionCount++;
...
function auditDesign(siteDir: string, brief: DesignBrief): CheckResult[] {
  ...
  results.push({
    ...
    message: layout.includes("next/font/google")
      ? `${brief.font.display} + ${brief.font.body}`
      : "No font imports found",
    severity: "error",
  });
  ...
  results.push({
    ...
    message: css.includes("--brand-primary")
      ? `palette: ${brief.palette.id}`
  ...
  results.push({
    ...
    passed: page.includes("@/components/") || page.includes("framer-motion"),
    message: page.includes("@/components/")
      ? "Shared components used"
      : page.includes("framer-motion")
        ? "Inline framer-motion animations"
        : "No component imports found",
    severity: "error",
  });
  ...
  const needsPhoto = ["photo-bg", "split", "blurred-reveal"].includes(
    brief.hero.variant,
  );
  if (needsPhoto) {
    const hasHeroImage =
      page.includes("heroImage") ||
      page.includes("backgroundImage") ||
      page.includes("foregroundImage");
    results.push({
      name: "Hero has image",
      passed: hasHeroImage,
      message: hasHeroImage
        ? "Hero image set"
        : `Hero variant "${brief.hero.variant}" needs an image but none found`,
      severity: "warning",
    });
  }
...
function auditCopyQuality(
  ...
  results.push({
    ...
    passed: !foundGeneric,
    message: foundGeneric
      ? `Generic pattern found: "${foundGeneric.source}"`
      : "Headline appears unique",
    severity: "warning",
  });
...
export function runBuildCheck(slug: string): {
  passed: boolean;
  output: string;
} {
  const siteDir = join(process.cwd(), "sites", slug);
