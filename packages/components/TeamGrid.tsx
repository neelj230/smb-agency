...
export function TeamGrid({ heading = 'Meet Our Team', subheading, members: rawMembers }: TeamGridProps) {
            ...
            <motion.div key={member.name} variants={staggerItem} className="text-center group">
              {member.image ? (
                ...
                <div className="aspect-[3/4] rounded-2xl bg-[var(--brand-primary)]/[0.07] mb-6 flex items-center justify-center">
                  <span className="font-display text-6xl text-[var(--brand-primary)]/20">{member.name.charAt(0)}</span>
                </div>
              )}
              <h3 className="font-display text-xl font-semibold text-[var(--brand-text)]">{member.name}</h3>
              <p className="text-sm text-[var(--brand-muted)] mt-1">{member.role}</p>
              ...
              {member.bio && <p className="mt-4 text-sm text-[var(--brand-muted)] leading-relaxed">{member.bio}</p>}
            </motion.div>
          ))}
        </motion.div>
      </div>
