      ...
      <section className="py-20 lg:py-28 px-6">
        <div className="max-w-3xl mx-auto">
          <motion.div {...fadeInUp} className="text-center mb-16 lg:mb-20">
            ...
            {subheading && <p className="mt-5 text-lg md:text-xl text-[var(--brand-muted)]">{subheading}</p>}
          </motion.div>
            ...
            <div className="space-y-14">
              {steps.map((step, i) => (
                <motion.div key={i} {...fadeInUp} className="flex gap-8">
                  ...
                  <div className="pt-2">
                    <h3 className="font-display text-xl md:text-2xl font-semibold text-[var(--brand-text)]">{step.title}</h3>
                    <p className="mt-3 text-base text-[var(--brand-muted)] leading-relaxed">{step.description}</p>
                    {step.image && (
        ...
        >
          {steps.map((step, i) => (
            <motion.div key={i} variants={staggerItem} className="relative">
              ...
              <h3 className="font-display text-xl md:text-2xl font-semibold text-[var(--brand-text)] mt-3">{step.title}</h3>
              <p className="mt-3 text-[var(--brand-muted)] text-base leading-relaxed">{step.description}</p>
              {step.image && (
