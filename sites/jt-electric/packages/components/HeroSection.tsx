...
function RatingBadge({
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-sm font-semibold ${light ? "text-white/90" : "text-[var(--brand-text)]"}`}
      >
        {rating.toFixed(1)}
      </span>
      {reviewCount && (
        <span
          className={`text-sm ${light ? "text-white/60" : "text-[var(--brand-muted)]"}`}
        >
          ({reviewCount} reviews)
        </span>
      )}
...
function PrimaryButton({
  ...
  const base =
    "inline-flex items-center px-8 py-4 rounded-full text-base font-semibold transition-all duration-200 hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl";
  const styles =
    variant === "white"
  ...
  if (variant === "video-bg" && videoSrc) {
    return (
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        >
          {rating && (
            <RatingBadge rating={rating} reviewCount={reviewCount} light />
          )}
          <motion.h1
            variants={fadeIn}
            className="font-display text-6xl md:text-8xl lg:text-9xl font-bold tracking-tighter leading-[0.9] mt-6"
          >
            {headline}
          </motion.h1>
          {subheadline && (
          ...
          >
            <PrimaryButton href={ctaHref} variant="white">
              {ctaText}
            </PrimaryButton>
            {secondaryCtaText && (
              <SecondaryButton href={secondaryCtaHref || "#services"} light>
                {secondaryCtaText}
              </SecondaryButton>
            )}
          </motion.div>
        ...
        <motion.div className="sticky top-0 h-screen flex items-center justify-center overflow-hidden">
          ...
          >
            ...
            {subheadline && (
              <motion.p
                variants={fadeIn}
                className="mt-8 text-lg md:text-xl text-white/70 max-w-2xl mx-auto italic leading-relaxed"
              >
                {subheadline}
              </motion.p>
            )}
        ...
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
          <motion.div variants={stagger} initial="initial" animate="animate">
            ...
            {subheadline && (
              <motion.p
                variants={fadeIn}
                className="mt-6 text-lg md:text-xl text-[var(--brand-muted)] max-w-lg leading-relaxed"
              >
                {subheadline}
              </motion.p>
            )}
            ...
            >
              <PrimaryButton href={ctaHref}>{ctaText}</PrimaryButton>
              {secondaryCtaText && (
                <SecondaryButton href={secondaryCtaHref || "#services"}>
                  {secondaryCtaText}
                </SecondaryButton>
              )}
            </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-6 w-full relative z-10">
          ...
          >
            ...
            {subheadline && (
              <motion.p
                variants={fadeIn}
                className="mt-8 text-xl md:text-2xl text-white/50 max-w-xl leading-relaxed"
              >
                {subheadline}
              </motion.p>
            )}
    ...
    <section className="min-h-[90vh] flex items-center justify-center relative overflow-hidden py-32">
      ...
      >
        ...
        {subheadline && (
          <motion.p
            variants={fadeIn}
            className="mt-8 text-lg md:text-xl text-[var(--brand-muted)] max-w-2xl mx-auto leading-relaxed"
          >
            {subheadline}
          </motion.p>
        )}
