    ...
    <div className="border-b border-black/[0.08] last:border-0">
      ...
      >
        <div className="flex items-center gap-4">
          <span className="font-mono text-sm text-[var(--brand-primary)] tracking-wider">
            {String(index + 1).padStart(2, "0")}
          </span>
          <h3 className="font-display text-lg md:text-xl font-semibold text-[var(--brand-text)]">
            {item.question}
          </h3>
        </div>
        <div
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <p className="pb-6 pl-14 pr-4 text-[var(--brand-muted)] text-base leading-relaxed">
              {item.answer}
            </p>
          </motion.div>
        )}
