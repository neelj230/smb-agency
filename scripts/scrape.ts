...
const SPREADSHEET_PATH =
  process.env.SOURCE_CSV_PATH || join(process.cwd(), "data", "spreadsheet.csv");
...
function findInSpreadsheet(slug: string): {
  name: string;
  category: string;
  address: string;
  phone: string;
...
async function downloadPhoto(
  ...
  filename: string,
  alt: string,
): Promise<{ src: string; alt: string; category: string } | null> {
  try {
 ...
 */
async function downloadExternalPhoto(
  externalUrl: string,
  filename: string,
  alt: string,
): Promise<{ src: string; alt: string; category: string } | null> {
 ...
 */
async function categorizePhotosWithAI(
  photos: { src: string; alt: string; category: string }[],
): Promise<{ src: string; alt: string; category: string }[]> {
  ...
  try {
    ...
    for (const photo of photos) {
      try {
        ...
        const mediaType = actualPath.endsWith(".webp")
          ? "image/webp"
          : "image/jpeg";
...
function mergeSourceData(
  ...
  if (yelpData) {
    ...
    const existingAuthors = new Set(
      merged.reviews.map((r) => r.author.toLowerCase()),
    );
  ...
  if (facebookData) {
    ...
    if (
      Object.keys(facebookData.hours).length > 0 &&
      Object.keys(merged.hours).length === 0
    ) {
...
async function main() {
  ...
  const parsedAddress = parseAddress(
    details.formatted_address || businessAddress,
  );
  ...
  // Process reviews
  const reviews = (details.reviews || []).map(
    (r: {
      text: string;
      author_name: string;
      rating: number;
  ...
  let businessData: BusinessData = {
    ...
    stats: [
      {
        value: details.user_ratings_total || 0,
        suffix: "+",
    ...
    analysis: {
      ...
      suggested_tagline: "",
      photo_quality:
        photos.length >= 5 ? "good" : photos.length >= 2 ? "fair" : "poor",
    },
    dataSources: ["google"],
  };
  ...
  if (websiteUrl) {
    ...
    if (websiteData && websiteData.photos.length > 0) {
      ...
      for (let i = 0; i < websitePhotos.length; i++) {
        ...
        const result = await downloadExternalPhoto(
          wp.src,
          `photo-${idx}.jpg`,
          wp.alt || `${businessName} website photo`,
        );
        if (result) photos.push(result);
      }
      if (photos.length > existingCount) {
        console.log(
          `    Downloaded ${photos.length - existingCount} website photos`,
        );
      }
    }
  } else {
  ...
  // ===== SOURCE 3: Yelp =====
  console.log("\n[3/5] Yelp...");
  const yelpData: YelpData | null = await scrapeYelp(
    businessName,
    businessData.city,
    businessData.state,
  );
  ...
  if (yelpData && yelpData.photos.length > 0 && photos.length < 10) {
    ...
    for (let i = 0; i < yelpPhotos.length; i++) {
      ...
      const result = await downloadExternalPhoto(
        yp.src,
        `photo-${idx}.jpg`,
        yp.alt || `${businessName} Yelp photo`,
      );
      if (result) {
        photos.push(result);
        yelpPhotoCount++;
    ...
    if (yelpPhotoCount > 0)
      console.log(`    Downloaded ${yelpPhotoCount} Yelp photos`);
  }
  ...
  const fbUrl =
    websiteData?.socialLinks?.facebook || businessData.socialLinks?.facebook;
  if (fbUrl) {
    facebookData = await scrapeFacebook(fbUrl);
  } else {
    // Try to discover Facebook page
    const discoveredFb = await findFacebookPage(
      businessName,
      businessData.city,
    );
    if (discoveredFb) {
  ...
  if (SHOULD_UPDATE_STATUS && existsSync(SPREADSHEET_PATH)) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (
        columns &&
        cols[columns.slug] === slug &&
        typeof columns.status === "number" &&
        columns.status >= 0
      ) {
  ...
  console.log(
    `  Reviews: ${businessData.reviews.length} (${businessData.reviews.filter((r) => r.source === "google").length} Google, ${businessData.reviews.filter((r) => r.source === "yelp").length} Yelp, ${businessData.reviews.filter((r) => (r as any).source === "facebook").length} Facebook)`,
  );
  console.log(`  Services: ${businessData.services.length}`);
  console.log(`  Team: ${businessData.team.length}`);
  console.log(
    `  Social links: ${Object.keys(businessData.socialLinks).length}`,
  );
  console.log(
    `  Brand colors: ${businessData.brandColors.primary || "pending AI enrichment"}`,
  );
