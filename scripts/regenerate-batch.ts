...
async function main() {
  ...
  const results: { slug: string; status: 'success' | 'failed'; error?: string; time: number }[] = []
  ...
  for (let i = 0; i < slugs.length; i++) {
    ...
    try {
      ...
      results.push({ slug, status: 'failed', error: message, time: parseFloat(elapsed) })
  ...
  for (const r of results) {
    ...
    console.log(`  ${icon} ${r.slug} (${r.time}s)${r.error ? ` — ${r.error.substring(0, 80)}` : ''}`)
  }
