...
const skipBuilt = args.includes("--skip-built");

const SOURCE_CSV_PATH = join(
  process.cwd(),
  "data",
...
function loadTopValidRows(count: number): SeedRow[] {
  ...
  if (skipBuilt) {
    const remaining = rows.filter(
      (row) =>
        !existsSync(join(process.cwd(), "sites", row.slug, "app", "page.tsx")),
    );
    console.log(
      `Skipping ${rows.length - remaining.length} already-built sites`,
    );
...
async function main() {
  ...
  console.log(
    `Using ${selected.length} Oregon businesses from ${SOURCE_CSV_PATH}`,
  );
  ...
  for (const row of selected) {
    try {
      ...
      console.error(`\n✗ FAILED: ${row.slug} — ${msg.split("\n")[0]}`);
      failed.push({ slug: row.slug, error: msg.split("\n")[0] });
    }
  }

  console.log("\n" + "=".repeat(60));
  console.log(`Batch complete: ${succeeded.length} succeeded, ${failed.length} failed out of ${selected.length}`);
  if (failed.length > 0) {
    console.log("\nFailed businesses:");
