...
if (!slug) {
  console.error(
    "Usage: npx tsx scripts/generate-placeholders.ts <business-slug>",
  );
  process.exit(1);
}
...
const business = JSON.parse(
  readFileSync(join(businessDir, "business.json"), "utf-8"),
);
...
async function main() {
  console.log(
    `Generating ${photos.length} placeholder images for ${business.name}...`,
  );
