...
export async function scrapeWebsite(url: string): Promise<WebsiteData | null> {
  try {
    ...
    const aboutPageData = await tryScrapePage($, baseUrl, [
      "about",
      "about-us",
      "our-story",
    ]);
    const teamPageData = await tryScrapePage($, baseUrl, [
      "team",
      "our-team",
      "staff",
      "meet-the-team",
    ...
    console.log(
      `    Services: ${services.length}, Team: ${fullTeam.length}, Photos: ${photos.length}`,
    );
    console.log(
      `    Social links: ${Object.keys(socialLinks).length}, Colors: ${brandColors.length}`,
    );
  ...
  for (const path of paths) {
    ...
    if (html && html.length > 500) {
      ...
      const mainContent = page$(
        'main, [role="main"], article, .content, .about, .story',
      )
        .first()
        .text()
        .trim();
  ...
  if (services.length === 0) {
    $("h2, h3").each((_, el) => {
      ...
      if (
        ...
        heading.includes("our work") ||
        heading.includes("menu") ||
        heading.includes("offerings")
      ) {
...
function extractTeam(
  ...
  for (const selector of teamSelectors) {
    $(selector).each((_, el) => {
      const $el = $(el);
      const name = $el
        .find("h3, h4, .name, .member-name")
        .first()
        .text()
        .trim();
      const role = $el
        .find(".role, .title, .position, .job-title, h5")
        .first()
        .text()
        .trim();
...
function extractPhotos(
  ...
  $("img").each((_, el) => {
    ...
    if (!src || src.includes("data:image/svg") || src.includes("pixel.gif"))
      return;
